...
export function StartMenu({ desktopApps, quickActions, onAppOpen, onLogout }) {
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 md:gap-4">
          {filteredApps.map((app) => (
            <button
              ...
              className="group p-4 md:p-6 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 transition-all duration-300 flex flex-col items-center gap-3 md:gap-4"
            >
              <div className="w-14 h-14 md:w-16 md:h-16 bg-gradient-to-br from-blue-500 to-purple-600 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform">
                <app.icon className="w-8 h-8 md:w-9 md:h-9 text-white" />
              </div>
              <span className="text-white text-sm md:text-base font-bold text-center line-clamp-2">
                {app.name}
              </span>
            </button>
          ))}
      ...
      <div className="p-3 md:p-6 border-t border-white/10">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 md:gap-3">
          {quickActions.map((action) => (
            <button
              key={action.id}
              className="flex items-center gap-3 md:gap-4 p-4 md:p-5 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 transition-all group"
            >
      ...
      <div className="p-3 md:p-6 border-t border-white/10 bg-white/5">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button className="w-10 h-10 md:w-12 md:h-12 bg-white/10 hover:bg-white/20 border border-white/20 rounded-xl flex items-center justify-center transition-all group">
              <Settings className="w-5 h-5 md:w-6 md:h-6 text-white/70 group-hover:text-white group-hover:rotate-90 transition-all duration-300" />
            </button>
            <button
              onClick={onLogout}
              className="w-10 h-10 md:w-12 md:h-12 bg-red-500/20 hover:bg-red-500/30 border border-red-500/50 rounded-xl flex items-center justify-center transition-all group"
            >
              <Power className="w-5 h-5 md:w-6 md:h-6 text-red-400 group-hover:text-red-300 transition-colors" />
            </button>
          </div>
        </div>
      </div>
