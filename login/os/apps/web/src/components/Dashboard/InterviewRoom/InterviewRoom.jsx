...
export function InterviewRoom({ interview, candidate, onEnd, mode }) {
  ...
  // Camera access for Real mode
  useEffect(() => {
    let stream = null;
    if (mode === "real" && isVideoOn) {
      ...
      <div className="bg-white/5 border-b border-white/10 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`p-3 rounded-lg transition-all ${isAudioOn
                ? "bg-white/10 hover:bg-white/20 text-white"
                : "bg-red-500/20 hover:bg-red-500/30 text-red-400"
              }`}
          >
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="h-2/5 bg-black relative">
            ...
            {/* Real Mode: Remote Stream Placeholder */}
            {mode === 'real' && (
              <div className="absolute bottom-4 right-4 w-48 h-36 bg-slate-800 rounded-lg border-2 border-white/20 overflow-hidden">
                <div className="w-full h-full flex items-center justify-center bg-black/50">
                  <span className="text-white/50 text-xs text-center p-2">Waiting for candidate to join...</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex-1 bg-white/5 p-4 overflow-y-auto">
            ...
            <div className="space-y-3">
              {transcript.map((entry) => (
                <div
                  key={entry.id}
                  className={`p-3 rounded-lg ${entry.speaker === "interviewer"
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span
                      className={`text-xs font-semibold ${entry.speaker === "interviewer"
                        ? "text-blue-400"
                        : "text-purple-400"
                        }`}
                    >
            ...
            <div className="space-y-3">
              ...
              <div className="bg-white/5 border border-white/10 rounded-lg p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span
                    className={`font-bold ${candidateAnalysis.credibilityScore >= 70
                      ? "text-green-400"
                ...
                <div className="w-full bg-white/10 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full ${candidateAnalysis.credibilityScore >= 70
                      ? "bg-green-500"
                      : candidateAnalysis.credibilityScore >= 40
                        ? "bg-yellow-500"
                        : "bg-red-500"
