...
export function Window({ window, onClose, onMinimize, children }) {
  ...
  return (
    <div
      className={`absolute z-20 ${isMaximized
      ...
      <div className="w-full h-full backdrop-blur-xl bg-white/10 border border-white/20 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        ...
        >
          <div className="flex items-center gap-3">
            <window.icon className="w-6 h-6" />
            <span className="font-semibold text-lg">{window.name}</span>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setIsMaximized(!isMaximized)}
              className="w-10 h-10 rounded-xl hover:bg-white/20 flex items-center justify-center transition-all"
